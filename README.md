...
- **自动合并进当前分支**：在 snapshot 之后执行。`agent-farm worker --auto-merge`（或 `AGENT_FARM_AUTO_MERGE=1`）。任务在 **自动 approve 并标记 done** 后，会在仓库根把对应 `agent-farm/<id>` **串行** `git merge --no-ff` 进**此时主工作区已检出的分支**；合并失败会写事件 **`task_merge_failed`**（任务仍为 done）。Wave 脚本默认开启合并，可用 **`AGENT_FARM_AUTO_MERGE=0`** 关闭。使用 **`--no-auto-approve-review`** 时不会走 done 自动合并路径，需自行合并。
