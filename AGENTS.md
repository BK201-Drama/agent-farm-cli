...
- **Wave → OpenCode（仅此）**：在 **`.agent-farm/waves/`** 写入 wave JSON（根为数组；每项为完整任务对象，经 `queue add --task-json` 入队，可含 `mode`、`priority`、`acceptance_criteria` 等，至少 `task_id` / `dedupe_key` / `prompt`）→ **`./scripts/agent-farm-dispatch-batch.sh <该文件>`** 或 **`npm run farm:wave -- …`**（无 Bash 时用 **`npm run farm:dispatch:batch:node --`**）。字段与形态可参考仓库 **`examples/agent-farm-waves/example-wave.json`**（可复制到 `.agent-farm/waves/` 再改）。`project init` 会创建空目录 `.agent-farm/waves/`。
